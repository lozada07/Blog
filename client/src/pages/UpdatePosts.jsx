...
import { createPost, updatePost, getPost } from "../services/post";
...
const UpdatePosts = () => {
  ...
  const {
    ...
    loading: loadingPost,
    response: post,
  } = useApiRequest(getPost);

  useEffect(() => {
    makeRequestPost(id);
  }, []);

  const { loading, error, makeRequest } = useApiRequest(updatePost);
  const initialCategories = useRef(post?.category);
  ...
  return (
    <div className="mt-0">
      ...
      <div className="max-w-3xl mx-auto bg-white p-5 rounded shadow-2xl ">
        <h1 className="h1 text-center mt-2 mb-14">Update Post</h1>
        {error && (
          <div className="mb-3">
            <Error error={error} />
          </div>
